...
const CategoryPage = () => {
  ...
  const {
    ...
    deleteCategory,
    isLoading,
    blogCategoryListData, // Matches store: resp?.data?.categories
    blogCategoryPaginator, // Matches store: resp?.data?.paginator
    blogCategoryCounts, // Ideally added to store from resp?.data?.counts
  } = useBlogStore();
  ...
  const currentStatus = searchParams.get('status') || ''; // 'true' or 'false'
