...
jobApplyQueue.process(async (job) => {
  ...
  console.log(
    `[JobApplyQueue] Processing application for student ${studentId} to job: ${jobData.title}`,
  );

  try {
    // 1. Fetch student data, including OAuth refresh token and name
    const student = await Student.findById(studentId).select(
      'fullName email google',
    );
    if (!student || !student.google || !student.google.refresh_token) {
      console.error(
        `[JobApplyQueue] Student ${studentId} not found or missing Google OAuth refresh token.`,
      );
      // If student data or token is missing, the job cannot proceed, so throw to mark as failed.
      throw new Error('Student data or Google OAuth token missing.');
    }
