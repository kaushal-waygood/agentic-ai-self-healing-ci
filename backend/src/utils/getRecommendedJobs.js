...
//         console.log('API response check values ... shariq khan...:', apiJobs);
...
//// currrent code througth shariq khan
...
import mongoose from 'mongoose';
import { config } from '../config/config.js';
console.log(config.rapidApiKey, 'check keyss');
...
export const getRecommendedJobs = async (
  ...
  try {
    ...
    console.log('Additional filter:', additionalFilter);

    const filter = { isActive: true };
    const searchTerms = [
      jobRole,
      additionalFilter.jobTitle,
      ...studentSkills.map((s) => s.skill),
      ...(additionalFilter.uploadedCVData?.skills?.map((s) => s.skill) || []),
      ...studentExperience
        .map((exp) => exp.title || exp.designation)
        .filter(Boolean),
      ...studentEducation
        .map((edu) => edu.fieldOfStudy || edu.degree)
        .filter(Boolean),
    ];
    ...
    if (preferences.isRemote || additionalFilter.isRemote) {
      filter.isRemote = true;
    }
    if (additionalFilter.country) {
      filter.country = { $regex: additionalFilter.country, $options: 'i' };
    } else if (preferences.preferredCountries?.length) {
      filter.country = {
        $in: preferences.preferredCountries.map((c) => new RegExp(c, 'i')),
      };
    }

    if (additionalFilter.employmentType) {
      filter.jobTypes = { $in: [additionalFilter.employmentType] };
    } else if (preferences.preferredJobTypes?.length) {
      filter.jobTypes = { $in: preferences.preferredJobTypes };
    }

    if (preferences.preferredSalary?.min) {
      const minSalary = convertSalaryToYearly(
    ...
    const maxExp = Math.max(
      ...studentExperience.map((exp) => exp.experienceYrs || 0),
      0,
    );
    filter.experience = { $lte: maxExp + 1 };

    const mongoJobs = await Job.find(filter).limit(limit * 2);
    let jobsWithScores = mongoJobs
      .map((job) => ({
        job: job.toObject ? job.toObject() : job,
        score: calculateMatchScore(
          job,
          { ...preferences, ...additionalFilter },
          jobRole,
          studentSkills,
          studentExperience,
          studentEducation,
    ...
    if (jobsWithScores.length === 0) {
      console.log('No matching jobs in MongoDB, trying external API...');
      const queryParams = {
        query: uniqueSearchTerms.length
          ? uniqueSearchTerms.join(' OR ')
          : additionalFilter.jobTitle || 'Software Engineer',
        remote_jobs_only:
          preferences.isRemote || additionalFilter.isRemote || false,
        country:
          additionalFilter.country?.toLowerCase() ||
          preferences.preferredCountries?.[0]?.toLowerCase() ||
          'in',
        employment_types:
          additionalFilter.employmentType?.toLowerCase() ||
          preferences.preferredJobTypes
            ?.map((type) => type.toLowerCase())
            .join(',') ||
          'fulltime',
        job_requirements:
          maxExp <= 3 ? 'under_3_years_experience' : 'no_experience',
      };

      try {
        const apiResponse = await axios.get(config.rapidJobApi, {
          params: { ...queryParams, num_pages: 1 },
          headers: {
            'X-RapidAPI-Key': config.rapidApiKey,
            'X-RapidAPI-Host': config.rapidApiHost,
          },
        });
        console.log(apiResponse.data, 'dsjbfsdhbfew');
        const apiJobs = apiResponse.data.data || [];
        jobsWithScores = apiJobs
          .map((apiJob) => {
            try {
    ...
    return finalJobs.slice(0, limit);
  } catch (error) {
    console.error('Error in getRecommendedJobs:', error);
    throw error;
  }
