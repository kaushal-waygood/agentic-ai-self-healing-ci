...
export const extractStudentDataFromCV = async (req, res) => {
  try {
    ...
    // Upload to Cloudinary (optional but kept)
    const originalName = (req.file.originalname || `cv-${Date.now()}`).replace(
      /\s+/g,
      '-',
    );
    ...
    const cvUrl = cloudResult?.secure_url || cloudResult?.url || null;

    // Write buffer to a temp file for your existing extractor
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'cv-'));
    const ext = path.extname(req.file.originalname) || '.pdf';
    const tmpFilePath = path.join(tmpDir, `${userId}-${Date.now()}${ext}`);
    fs.writeFileSync(tmpFilePath, req.file.buffer);

    // Run extractor (with retry if you have it)
    let extractedData;
    try {
      extractedData = await retryOperation(() =>
        extractDataFromCV(tmpFilePath),
      );
    } finally {
      // cleanup temp file/dir regardless of extractor outcome
      try {
        fs.unlinkSync(tmpFilePath);
        fs.rmdirSync(tmpDir, { recursive: true });
      } catch (e) {
        console.warn('Temp cleanup failed:', e && e.message);
      }
    }
    ...
    const updatedStudent = await Student.findByIdAndUpdate(
      userId,
      {
        ...
        projects: extractedData?.projects || [],
        jobPreferences: extractedData?.jobPreferences || {},
        uploadedCV: req.file.secure_url || req.file.url,
        ...(cvUrl ? { cvUrl } : {}),
      },
      { new: true, runValidators: true },
    );
    ...
    if (err.status === 503) {
      return res
        .status(503)
        .json({ error: 'AI service busy. Try again later.' });
    }
