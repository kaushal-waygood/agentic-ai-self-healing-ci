...
// Context Import (From your Dashboard Layout)
import { useSidebar } from '@/app/dashboard/layout';

// ZUSTAND Import (Replaces Redux)
...
export const AppSidebarContent = ({
  ...
  // 1. Access Sidebar Context
  const { isPinned, setPinned } = useSidebar();

  const pathname = usePathname();
  const router = useRouter();

  // 2. REPLACED: Redux with Zustand
  // We extract 'user' directly from the store
  ...
  // Construct local user object for UI rendering
  const user = {
  ...
  return (
    <div className="h-full w-full flex flex-col relative bg-gradient-to-br from-slate-50 to-white border-r border-slate-200">
      {/* Dynamic Background */}
