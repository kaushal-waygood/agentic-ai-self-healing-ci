...
import { JobListingSchema, type JobListing, mockJobListings } from '@/lib/data/jobs';
...
export async function getJobDetails(input: GetJobDetailsInput): Promise<GetJobDetailsOutput | null> {
...
const getJobDetailsFlow = ai.defineFlow(
  ...
  async ({ jobId }) => {
    ...
    if (cachedJob && cachedJob.description && cachedJob.description.includes('<h4>')) {
      console.log(`[getJobDetailsFlow] Returning formatted job ${jobId} from cache.`);
      return cachedJob;
    }

    try {
        // 2. If not in cache or not formatted, fetch from the API
        console.log(`[getJobDetailsFlow] Job ${jobId} not in cache or unformatted. Fetching from API.`);
        const api = await JSearchApiService.getInstance();
        const jobDetails = await api.getJobDetails(jobId);

        if (!jobDetails) {
            console.error(`[getJobDetailsFlow] Failed to get details for job ${jobId}.`);
            return null;
        }
        ...
        try {
            const formatResult = await formatJobDescription({ rawDescription: jobDetails.description });
            formattedDescription = formatResult.formattedDescription;
        } catch (error) {
            console.error(`[getJobDetailsFlow] Failed to format description for job ${jobId}, using fallback.`, error);
        ...
        console.log(`[getJobDetailsFlow] Updated job ${jobId} in cache.`);
        } else {
        mockJobListings.push(finalJob);
        console.log(`[getJobDetailsFlow] Added new job ${jobId} to cache.`);
        }
        ...
        return finalJob;
         ...
         if (error instanceof JSearchApiError) {
            console.error(`[getJobDetailsFlow] JSearch API is down. Could not fetch job ${jobId}.`, error.message);
        } else {
            console.error(`[getJobDetailsFlow] An unexpected error occurred while fetching job ${jobId}.`, error);
        }
