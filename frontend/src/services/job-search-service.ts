...
export class JSearchApiService {
  ...
  private async makeRequest(endpoint: string, params: URLSearchParams, attempt = 1): Promise<any> {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if ((response.status === 401 || response.status === 429 || response.status >= 500) && attempt < totalKeys) {
            console.warn(`JSearch API Error (${response.status}), retrying with next key. Attempt ${attempt + 1}/${totalKeys}`);
        ...
        throw new JSearchApiError("The job search service is currently unavailable.");
      }
      ...
      return data;
       ...
       console.error(`[JSearchService] Network or other error during fetch to ${endpoint}:`, error);
       if (attempt < totalKeys) {
         console.log(`Retrying request due to fetch error. Attempt ${attempt + 1}/${totalKeys}`);
         return this.makeRequest(endpoint, params, attempt + 1);
       }
       // Final attempt failed
       const finalErrorMessage = `JSearch API is currently unavailable due to a network error.`;
       console.error(`[JSearchService] Fetch request failed after all retries for endpoint ${endpoint}.`);
  ...
  public async searchJobs(filters: JobSearchFlowInput): Promise<JobListing[]> {
    ...
    if (filters.datePosted && filters.datePosted !== 'all') params.append('date_posted', filters.datePosted);
    if (filters.workFromHome) params.append('remote_jobs_only', 'true');
    if (filters.employmentTypes?.length) params.append('employment_types', filters.employmentTypes.join(','));
    if (filters.jobRequirements?.length) params.append('job_requirements', filters.jobRequirements.join(','));
    ...
    return apiData.map((job: any): JobListing => ({
      id: job.job_id,
      title: job.job_title,
      ...
      postedDate: job.job_posted_at_datetime_utc ? new Date(job.job_posted_at_datetime_utc).toLocaleDateString() : 'N/A',
      description: job.job_description,
      status: 'published', // All jobs from JSearch are considered published
      salary: job.job_salary_period && job.job_salary_currency ? `${job.job_min_salary}-${job.job_max_salary} ${job.job_salary_currency} ${job.job_salary_period}` : null,
  ...
  public async estimateSalary(jobTitle: string, location: string, experience: string): Promise<EstimateSalaryOutput | null> {
    const params = new URLSearchParams({
        job_title: jobTitle,
        location: location,
        location_type: 'ANY',
    ...
    if (!salaryData || salaryData.min_salary === undefined || salaryData.min_salary === null) {
      console.log(`No salary data found for ${jobTitle} in ${location}.`);
      return null;
    }
