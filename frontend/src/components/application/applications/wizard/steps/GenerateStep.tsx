...
export const GenerateStep = ({
  ...
  const contextItems = [
    ...
    },
    {
      ...
      label: 'CV Source',
      value: cvContext?.name || 'No CV selected',
      sublabel: 'Your saved cv', // Kept static sublabel structure from ref
      iconBg: 'bg-blue-100',
      iconText: 'text-blue-600',
    },
    {
      ...
      <div className="mx-auto flex h-full w-full max-w-[850px] flex-col">
        ...
        <div className="mb-6 shrink-0">
          <div className="mb-6 text-center">
            <h1 className="text-[22px] font-black tracking-tight text-black md:text-[28px]">
              Ready to Generate?
            </h1>
            <p className="mt-1 text-[14px] font-medium text-slate-500">
              We'll use the provided info to tailor your application documents
              with AI precision.
            </p>
          </div>
        ...
        {/* MAIN WIZARD CARD */}
        <div className="mx-auto flex w-full max-w-[600px] flex-col overflow-hidden rounded-[20px] border border-slate-200 bg-white shadow-sm animate-in fade-in duration-500">
          {/* Card Header */}
          <div className="flex shrink-0 items-center gap-2 border-b border-slate-100 px-6 py-5">
            <CheckCircle2
          ...
          {/* Content Area */}
          <div className="custom-scrollbar flex min-h-0 flex-1 flex-col gap-3 overflow-y-auto p-6">
            {contextItems.map((item, index) => {
              const Icon = item.icon;
              return (
                <div
                  key={index}
                  className="flex items-center gap-4 rounded-2xl border border-slate-100 bg-slate-50/50 p-4 transition-colors hover:bg-slate-50 animate-in slide-in-from-bottom-2"
                  style={{ animationDelay: `${index * 100}ms` }}
                >
                  <div
                    className={`flex h-12 w-12 shrink-0 items-center justify-center rounded-[14px] ${item.iconBg} ${item.iconText}`}
                  >
          ...
          <div className="flex shrink-0 items-center justify-between border-t border-slate-100 bg-white px-6 py-5">
            ...
            <button
              onClick={handleGenerate}
              disabled={isLoading}
              className="group flex items-center gap-2 rounded-xl bg-blue-600 px-6 py-3 text-[14px] font-extrabold text-white shadow-[0_4px_16px_rgba(37,99,235,0.25)] transition-all hover:-translate-y-0.5 hover:bg-blue-700 disabled:opacity-70 disabled:hover:translate-y-0"
            >
              {isLoading ? (
                <>
