...
//   const hasAgents = agents.autoPilot && agents.autoPilot.length > 0;
...
const Step0_Intro = ({ nextStep, agents = {}, onEdit, onDelete }) => {
  ...
  // ✅ Safely handle agents structure
  const hasAgents =
    Array.isArray(agents?.autoPilot) && agents.autoPilot.length > 0;
  const totalAgents = hasAgents ? agents.autoPilot.length : 0;
        ...
        <div className="p-2">
          {!hasAgents ? (
            ...
            <div className="space-y-4 animate-slide-up">
              ...
              {/* Agent List */}
              <ul className="space-y-3">
                {agents.autoPilot.map((agent, idx) => (
                  <li
