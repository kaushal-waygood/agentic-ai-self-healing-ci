...
import { Crown, Zap, ChevronDown, Calendar } from 'lucide-react';
...
const PlanDropdown = ({
  ...
  return (
    <div className="relative">
      {/* Redesigned Plan Toggle Button */}
      <button
        onClick={onToggle}
        // className="flex h-10 items-center gap-2 rounded-xl border border-slate-200/80 bg-white px-3.5 shadow-sm transition-all hover:border-blue-300 hover:bg-blue-50"
        className={config.buttonClasses}
      >
        {/* If your config.buttonClasses passed colors, you can merge them, otherwise this neutral/blue theme looks very premium */}
        <Icon className="h-4 w-4 text-indigo-500" strokeWidth={2.5} />
      ...
      {isOpen && (
        ...
        >
          ...
          <div className={config.headerClasses}>
            <div className="flex items-center space-x-3 text-white px-4 py-4">
          ...
          {(planType === 'Free' || planType === 'Weekly') && (
            <div className="border-t border-slate-100 p-3 bg-slate-50/50">
              <button
                className="flex w-full items-center justify-center space-x-2 rounded-xl bg-blue-600 py-2.5 text-[13px] font-bold text-white shadow-[0_4px_12px_rgba(37,99,235,0.25)] transition-all hover:-translate-y-0.5 hover:bg-blue-700"
