...
describe("Student Tests", () => {
    ...
    it("should update student profile", async () => {
        ...
        const res = await axios.patch("/api/v1/students/profile/update", updateData);
        expect(res.status).toBe(200);
    ...
    it("should remove experience", async () => {
        ...
        const res = await axios.delete(`/api/v1/students/experience/remove/${experienceId}`);
        expect(res.status).toBe(200);
        const experiences = res.data.experience || [];
    ...
    it("should remove education", async () => {
        ...
        const res = await axios.delete(`/api/v1/students/education/remove/${educationId}`);
        expect(res.status).toBe(200);
        const education = res.data.education || [];
    ...
    it("should add project", async () => {
        ...
        const addedProj = projects.find(p => p.projectName === projData.projectName);
    ...
    it("should remove project", async () => {
        if (!projectId) return;
        const res = await axios.delete(`/api/v1/students/project/remove/${projectId}`);
        expect(res.status).toBe(200);
        const projects = res.data.projects || [];
